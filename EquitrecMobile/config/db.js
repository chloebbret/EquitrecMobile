...
// Exécute les requêtes SQL pour créer les tables et insérer les données
db.serialize(() => {
    // Table: competiteur
    ...
    // Insertion des données dans la table competiteur
    ...
    // Table: juge
    db.run(`CREATE TABLE IF NOT EXISTS juge (
    ...
    db.run(`INSERT INTO juge (id_juge, nom_juge, prenom_juge, tel_juge, mail_juge, login_juge, pass_juge, id_competition) VALUES
    ...
    // Récupère les données de la table competiteur
    ...
    db.all(`SELECT * FROM juge`, (err, rows) => {
        if (err) {
            console.error(err);
        } else {
            rows.forEach((row) => {
